  ...
  override def receive: Receive = { case soapRequest: SoapRequest =>
    ...
    reFlow = Some(
      Re(
        ...
        flowAction = Some(FlowActionEnum.CREATE_FLOW.toString)
      )
    )
    ...
    pipeline.recover({
      ...
      traceInterfaceRequest(soapRequest, reFlow.get.copy(flowAction = Some(FlowActionEnum.ADD_PAYMENT.toString)), soapRequest.reExtra, reEventFunc, ddataMap)
      traceInterfaceRequest(soapRequest, reFlow.get.copy(flowAction = Some(FlowActionEnum.PUBLISH.toString)), soapRequest.reExtra, reEventFunc, ddataMap)
