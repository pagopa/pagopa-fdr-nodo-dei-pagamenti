  ...
  override def receive: Receive = { case soapRequest: SoapRequest =>
    ...
    reFlow = Some(
      Re(
        ...
        flowAction = Some(FlowActionEnum.GET_FDR.toString)
      )
    )
    ...
    pipeline.recover({
      ...
      traceInterfaceRequest(soapRequest, reFlow.get.copy(flowAction = Some(FlowActionEnum.GET_FDR_PAYMENT.toString)), soapRequest.reExtra, reEventFunc, ddataMap)
