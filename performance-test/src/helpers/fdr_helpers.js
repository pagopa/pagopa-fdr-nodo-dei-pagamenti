...
export function generateNodoInviaFlussoRendicontazioneFixedPayments(parameters, flow_id, flow_size, multiplePaymentsObject) {
    ...
    var report = `<pay_i:FlussoRiversamento xmlns:pay_i="http://www.digitpa.gov.it/schemas/2011/Pagamenti/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://www.digitpa.gov.it/schemas/2011/Pagamenti/ FlussoRendicontazione_v_1_0_1.xsd ">
        ...
        <pay_i:importoTotalePagamenti>${multiplePaymentsObject.total}.00</pay_i:importoTotalePagamenti>
        ${multiplePaymentsObject.multiplePayments}
...
export function generateMultiplePaymentsObject(parameters, flow_size) {
    ...
    for (let i = 1; i <= flow_size; i ++) {
        ...
        total += amount;
        var suffix = ('0000000000' + i).slice(-10);
        var singlePayment = `<pay_i:datiSingoliPagamenti><pay_i:identificativoUnivocoVersamento>IUV${suffix}</pay_i:identificativoUnivocoVersamento><pay_i:identificativoUnivocoRiscossione>IUR${suffix}</pay_i:identificativoUnivocoRiscossione><pay_i:indiceDatiSingoloPagamento>1</pay_i:indiceDatiSingoloPagamento><pay_i:singoloImportoPagato>${amount}.00</pay_i:singoloImportoPagato><pay_i:codiceEsitoSingoloPagamento>0</pay_i:codiceEsitoSingoloPagamento><pay_i:dataEsitoSingoloPagamento>${parameters.today}</pay_i:dataEsitoSingoloPagamento></pay_i:datiSingoliPagamenti>`;
        multiplePayments = multiplePayments.concat(singlePayment);
    }
    return {multiplePayments: multiplePayments, total:total}
}
